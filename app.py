    ...
    # --- OpenAI ---
        ...
        try:
            ...
            if image_data:
                ...
                model_to_use = (
                    "gpt-3.5-turbo"
                    if model_name == "default" or "gpt" not in model_name
                    else model_name
                )
    ...
    try:
        ...
        # 3. Handle Images (OCR)
            ...
            with st.spinner(f"Reading image..."):
                ...
                text = call_llm(
                    provider, ocr_model_signal, user_api_key, prompt, image_data=image
                )
...
def get_company_story(company, position):
    ...
    return stories.get(
        company,
        f"{company} is a leading technology firm dedicated to innovation and excellence in the **{position}** field. We value creativity, integrity, and a passion for building the future.",
...
def evaluate_interview(provider, api_key, cv_text, q_a_history, position, company_name):
    ...
    try:
        ...
        spec_score = (
            (data.get("specialized_correct_count", 0) / total_spec * 10)
            if total_spec > 0
            else 0
        )
        ...
        att_score = (
            (data.get("attitude_accepted_count", 0) / total_att * 10)
            if total_att > 0
            else 0
        )
        ...
        think_score = (
            (data.get("coding_accepted_count", 0) / total_code * 10)
            if total_code > 0
            else 0
        )
...
if "scores" not in st.session_state:
    st.session_state.scores = {"cv": 0, "specialized": 0, "attitude": 0, "thinking": 0}
if "history" not in st.session_state:
    st.session_state.history = {"specialized": [], "attitude": [], "coding": []}
...
with st.sidebar:
    ...
    if uploaded_file:
        ...
        position = st.selectbox(
            "Job Position", job_list, index=None, placeholder="Select a position..."
        )
        ...
        if position:
            ...
            if "target_company" not in st.session_state:
                st.session_state.target_company = random.choice(
                    ["NVIDIA", "Intel", "IBM", "AMD", "Meta"]
                )  # Updated Facebook to Meta
            ...
            with col2:
                reset_btn = st.button(
                    "RESET", disabled=(st.session_state.step == "setup")
                )
...
if "reset_btn" in locals() and reset_btn:
    ...
    for key in list(st.session_state.keys()):  # Iterate over a copy of keys
        if key not in [
            "target_company"
        ]:  # Keep the company assigned until the start flow
            del st.session_state[key]
    st.rerun()

# We check for mandatory fields before proceeding to CV Review (Application Submitted)
if (
    "uploaded_file" in locals()
    and uploaded_file
    and "position" in locals()
    and position
    and st.session_state.step == "setup"
):
...
# --- STEP 1: CV REVIEW (Wait Screen) ---
    ...
    if missing_elements and not demo_mode:
        st.error(
            f"❌ Application Rejected. Missing required elements: {', '.join(missing_elements)}"
        )
        ...
        time.sleep(1.5)
        st.session_state.scores["cv"] = (
            8.5  # Simulated score (will be updated by AI later)
        )
...
# --- STEP 2: SPECIALIZED INTRO (NEW INTRODUCTION SCREEN) ---
    ...
    with col2:
        ...
        st.warning(
            'Press **"Start"** if you are ready for the interview, or use the **"RESET"** button in the sidebar if you want to restart the application process.'
        )
    ...
    if st.session_state.current_q_idx < st.session_state.q_count:
        ...
        if f"q_spec_{q_num}" not in st.session_state:
            with st.spinner(f"Generating Technical Question {q_num}..."):
                ...
                prev_questions = [
                    item["question"]
                    for item in st.session_state.history.get("specialized", [])
                ]
        ...
        st.markdown(
            f'<div id="custom_timer_div" class="timer-display">⏱️ Time Left: {spec_time_str}</div>',
            unsafe_allow_html=True,
        )
        ...
        if options:
            answer = st.radio(
                "Select an Answer:", options, key=f"ans_spec_{q_num}", index=None
            )
        else:
        ...
        if st.button("Next Question"):
            if not answer:
                ...
                st.session_state.history["specialized"].append(
                    {"question": q_content, "answer": answer}
                )
...
# --- STEP 3: ATTITUDE ---
    ...
    if st.button("Start Attitude Test"):
        ...
        st.session_state.q_count_att = (
            10 if st.session_state.get("demo_mode", True) else 30
        )
    ...
    if st.session_state.current_q_att_idx < st.session_state.q_count_att:
        ...
        if f"q_att_{q_num}" not in st.session_state:
            with st.spinner(f"Generating Behavioral Question {q_num}..."):
                ...
                prev_questions = [
                    item["question"]
                    for item in st.session_state.history.get("attitude", [])
                ]
                prev_q_text = " ".join(prev_questions)
        ...
        # Show Timer AFTER content is loaded
        st.markdown(
            f'<div id="custom_timer_div" class="timer-display">⏱️ Time Left: {att_time_str}</div>',
            unsafe_allow_html=True,
        )
        ...
        if options:
            answer = st.radio(
                "Select an Answer:", options, key=f"ans_att_{q_num}", index=None
            )
        else:
        ...
        if st.button("Next"):
            if not answer:
                ...
                # Save answer for grading
                st.session_state.history["attitude"].append(
                    {"question": q_content, "answer": answer}
                )
...
# --- STEP 4: CODING CHALLENGE ---
    ...
    st.markdown(
        "**Allowed Languages:** Python, C++, Java, JavaScript, Go, Ruby, PHP, C#, Swift, Kotlin."
    )

    if st.button("Start Coding Challenge"):
        st.session_state.q_count_code = (
            1 if st.session_state.get("demo_mode", True) else 3
        )
    ...
    if st.session_state.current_q_code_idx < st.session_state.q_count_code:
        ...
        if f"q_code_{q_num}" not in st.session_state:
            with st.spinner(f"Generating Coding Problem {q_num}..."):
                ...
                prev_questions = [
                    item["question"]
                    for item in st.session_state.history.get("coding", [])
                ]
                prev_q_text = " ".join(prev_questions)
        ...
        # Show Timer AFTER content is loaded
        st.markdown(
            f'<div id="custom_timer_div" class="timer-display">⏱️ Time Left: {code_time_str}</div>',
            unsafe_allow_html=True,
        )
        ...
        code_text_input = st.text_area(
            "Or type code here:", height=200, key=f"text_{q_num}"
        )
        ...
        if st.button("Submit Code"):
            if not code_file and not code_text_input:
                ...
                ans_content = (
                    code_text_input
                    if code_text_input
                    else f"File uploaded: {code_file.name}"
                )
                st.session_state.history["coding"].append(
                    {
                        "question": st.session_state[f"q_code_{q_num}"],
                        "answer": ans_content,
                        "language": language,
                    }
                )
...
# --- STEP 5: FINAL EVALUATION (UPDATED WITH REAL SCORING & TIMER) ---
    ...
    if "eval_complete" not in st.session_state:
        ...
        user_key = st.session_state.get(
            "user_api_key", get_key("GEMINI_API_KEY") or get_key("GROQ_API_KEY")
        )
        ...
        with st.spinner("Finalizing report..."):
            ...
            st.session_state.scores["specialized"] = evaluation_result.get(
                "specialized_score", 0
            )
            st.session_state.scores["attitude"] = evaluation_result.get(
                "attitude_score", 0
            )
            st.session_state.scores["thinking"] = evaluation_result.get(
                "thinking_score", 0
            )
            st.session_state.final_status = evaluation_result.get("status", "NOT HIRED")
            st.session_state.final_feedback = evaluation_result.get(
                "feedback_markdown", "No feedback provided."
            )
    ...
    if st.session_state.get("eval_complete"):
        avg_score = (
            sum(st.session_state.scores.values()) / 4
        )  # Display simple average for user clarity
